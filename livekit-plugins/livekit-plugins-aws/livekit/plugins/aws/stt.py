...
class SpeechStream(stt.SpeechStream):
    ...
    async def _run(self) -> None:
        # Iterator persists across reconnections to avoid dropping audio frames
    ...
    async def _run_single_session(self, input_iterator: _RecognitionIterator) -> None:
        config = (
            Config(**asdict(self._credentials), region=self._opts.region)
            if isinstance(self._credentials, Credentials)
            else Config(
        ...
        stream = await client.start_stream_transcription(
            input=StartStreamTranscriptionInput(
                **{k: v for k, v in live_config.items() if is_given(v) and v is not None},
            ),
        )
        ...
        async with asyncio.TaskGroup() as tg:
            tg.create_task(self._send_audio_loop(input_iterator, stream.input_stream))
            tg.create_task(self._receive_transcript_loop(output_stream))
...
def _should_retry(exc: BaseException) -> bool:
    """
    Determines if the exception is a transient network/limit error
    that warrants a reconnection. Handles ExceptionGroups from TaskGroup.
    """
    # Errors we expect from AWS timeouts (4h limit) or network drops
